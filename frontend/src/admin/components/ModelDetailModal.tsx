export default function ModelDetailModal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative bg-white rounded-2xl shadow-2xl w-full max-h-[90vh] flex flex-col overflow-hidden animate-fade-in-up ${modalType === 'market' ? 'max-w-6xl h-[85vh]' : 'max-w-3xl'}`}>
        <div className="flex justify-between items-center p-5 border-b border-gray-100 bg-gray-50/50">
          <h3 className="font-bold text-xl text-gray-800 flex items-center">
            {modalType === 'local' ? (
              ...
              <><svg className="w-5 h-5 mr-2 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" /></svg> Civitai 模型页面: {modalData.name}</>
            )}
          </h3>
          ...
          {modalType === 'local' ? (
            <div className="space-y-6">
              <div className="flex items-start space-x-6">
                <div className="w-32 h-32 bg-gray-100 rounded-xl flex items-center justify-center flex-shrink-0 border border-gray-200 overflow-hidden">
                  {modalData.image_url ? (
            ...
            <div className="w-full h-full flex flex-col">
              <div className="bg-blue-50 border-b border-blue-100 p-3 flex items-center justify-between flex-shrink-0 px-6">
                <p className="text-sm text-blue-800 flex items-center">
                  <svg className="w-5 h-5 text-blue-500 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  正在展示 Civitai 原生页面，如果加载缓慢请检查网络。
                </p>
                <div className="flex space-x-3">
                  <a href={`https://civitai.com/models/${modalData.id || ''}`} target="_blank" rel="noreferrer" className="text-sm font-medium text-blue-600 hover:text-blue-800 bg-white border border-blue-200 px-3 py-1.5 rounded-lg hover:bg-blue-50 transition-colors flex items-center">
                    在新标签页打开
                  </a>
                  {handleDownload && (
                    <button
                      onClick={() => handleDownload(modalData)}
                      className="px-4 py-1.5 bg-blue-600 text-white rounded-lg text-sm font-bold hover:bg-blue-700 transition-colors shadow-sm"
                    >
                      下载到 ComfyUI
                    </button>
                  )}
              ...
              <iframe
                src={`https://civitai.com/models/${modalData.id || ''}`}
                className="w-full flex-1 border-0 min-h-[500px]"
                title="Civitai Model Page"
              />
            </div>
          )}
        </div>
